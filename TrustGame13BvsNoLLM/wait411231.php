...
@media (max-width: 768px) {
  ...
  p {
      font-size: 16px; /* Reduce paragraph font size for smaller devices */
        ...
        var defNext = 411232;
        var nextPageLoop;
        if (parseInt(loopEnd)==411231) {nextPageLoop='stage'+loopStart.toString()+'.php?session_index=<?php echo $_SESSION[sessionID];?>';}
...
if (transfer2 == 0) {
    setTimeout(function() {
        let currentUrl = window.location.href;
        let newUrl = currentUrl.replace(/wait\d+\.php/i, "stage411232.php");
        window.location.href = newUrl;
    }, 20000);
