...
@media (max-width: 768px) {
  ...
  p {
      font-size: 16px; /* Reduce paragraph font size for smaller devices */
        ...
        var defNext = 411237;
        var nextPageLoop;
        if (parseInt(loopEnd)==411236) {nextPageLoop='stage'+loopStart.toString()+'.php?session_index=<?php echo $_SESSION[sessionID];?>';}
...
if (transfer4 == 0) {
    setTimeout(function() {
        let currentUrl = window.location.href;
        let newUrl = currentUrl.replace(/wait\d+\.php/i, "stage411237.php");
        window.location.href = newUrl;
    }, 20000);
