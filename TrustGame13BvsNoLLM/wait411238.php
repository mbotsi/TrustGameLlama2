...
@media (max-width: 768px) {
  ...
  p {
      font-size: 16px; /* Reduce paragraph font size for smaller devices */
        ...
        var defNext = 411239;
        var nextPageLoop;
        if (parseInt(loopEnd)==411238) {nextPageLoop='stage'+loopStart.toString()+'.php?session_index=<?php echo $_SESSION[sessionID];?>';}
...
if (transfer5 == 0) {
    setTimeout(function() {
        let currentUrl = window.location.href;
        let newUrl = currentUrl.replace(/wait\d+\.php/i, "stage411239.php");
        window.location.href = newUrl;
    }, 20000);
