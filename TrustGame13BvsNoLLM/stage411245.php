  ...
  p {
      font-size: 16px; /* Reduce paragraph font size for smaller devices */
...
}</style><title>Redirect to Prolific - Dropout because partner left</title>
        ...
        function skipStage(proceedifpossible) {
         ...
         if (proceedifpossible) location.replace('stage411246.php?session_index=<?php echo $_SESSION[sessionID];?>');
         else location.replace('wait411245.php?session_index=<?php echo $_SESSION[sessionID];?>');
        }
...
setTimeout(function() {
    window.location.href = "https://app.prolific.com/submissions/complete?cc=C1OE0D20";
...
<h1><b>Your partner left.&nbsp;</b></h1><h1><b>Unfortunately the game cannot continue.&nbsp;</b><b style="color: inherit; font-family: inherit;">You are now being redirected to Prolific.</b></h1></div>
