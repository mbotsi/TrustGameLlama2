...
@media (max-width: 768px) {
  ...
  p {
      font-size: 16px; /* Reduce paragraph font size for smaller devices */
        ...
        var defNext = 411234;
        var nextPageLoop;
        if (parseInt(loopEnd)==411233) {nextPageLoop='stage'+loopStart.toString()+'.php?session_index=<?php echo $_SESSION[sessionID];?>';}
...
if (transfer3 == 0) {
    setTimeout(function() {
        let currentUrl = window.location.href;
        let newUrl = currentUrl.replace(/wait\d+\.php/i, "stage411234.php");
        window.location.href = newUrl;
    }, 20000);
