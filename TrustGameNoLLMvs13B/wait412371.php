...
@media (max-width: 768px) {
  ...
  p {
      font-size: 16px; /* Reduce paragraph font size for smaller devices */
        ...
        var defNext = 412372;
        var nextPageLoop;
        if (parseInt(loopEnd)==412371) {nextPageLoop='stage'+loopStart.toString()+'.php?session_index=<?php echo $_SESSION[sessionID];?>';}
...
if (transfer6 == 0) {
    setTimeout(function() {
        let currentUrl = window.location.href;
        let newUrl = currentUrl.replace(/wait\d+\.php/i, "stage412372.php");
        window.location.href = newUrl;
    }, 20000);
