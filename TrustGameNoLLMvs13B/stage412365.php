  ...
  p {
      font-size: 16px; /* Reduce paragraph font size for smaller devices */
        ...
        function skipStage(proceedifpossible) {
         ...
         if (proceedifpossible) location.replace('stage412366.php?session_index=<?php echo $_SESSION[sessionID];?>');
         else location.replace('wait412365.php?session_index=<?php echo $_SESSION[sessionID];?>');
        }
...
setTimeout(function() {
    ...
    let newUrl = currentUrl.replace(/stage412365\.php/i, "stage412366.php");
    changeRole();
    window.location.href = newUrl;
