...
class XLSXWriter
{
    ...
    private function addCellFormat($cell_format)
    {
        ...
        //       $cell_format='0'; // Change by Lucas: standard it should be an integer (or a number at least)
    ...
    protected function writeCell(XLSXWriter_BuffererWriter &$file, $row_number, $column_number, $value, $cell_format_index)
    {
        ...
        } else if (!is_string($value) || is_numeric($value)) { // tweaked by Lucas to write the numbers right
            $file->write('<c r="' . $cell_name . '" s="' . $cell_format_index . '" t="n"><v>' . ($value * 1) . '</v></c>');
        } else if ($value[0] != '0' && $value[0] != '+' && filter_var($value, FILTER_VALIDATE_INT, ['options' => ['max_range' => 2147483647]])) {
            $file->write('<c r="' . $cell_name . '" s="' . $cell_format_index . '" t="n"><v>' . ($value * 1) . '</v></c>');
    ...
    protected function writeStylesXML()
    {
        ...
        $file->write('<font><name val="Calibri"/><charset val="1"/><family val="2"/><sz val="10"/></font>');
